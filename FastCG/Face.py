...
from sklearn.neighbors import kneighbors_graph, radius_neighbors_graph
 ...
 # This one implements the FACE method from CARLA pacakge which in turn implements it from
...
class Face():
    ...
    def choose_random_subset(data, frac, index):
        """
        Choose a subset of data for computational efficiency for the instance in dataset
    ...
    def build_constraints(data, i, keys_immutable, epsilon=0.5):
        """
        Building adjacency matrices from immutable keys
        Parameters
        ----------
        data: pd.DataFrame
        i : int
